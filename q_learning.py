...
from ponggame import Plat, Ball, AI
import numpy as np
import ponggame
...
class Env(gym.Env):
    def __init__(self):
        ...
        self.plat_1 = Plat(pygame.image.load(os.path.join('Sprites\\table.png')).convert_alpha(), 760, ponggame.start_pos, 20, 100, ponggame.points_1, ponggame.table_speed, 0, 0)
        self.plat_2 = Plat(pygame.image.load(os.path.join('Sprites\\table.png')).convert_alpha(), 20, ponggame.start_pos, 20, 100, ponggame.points_2, ponggame.table_speed, 0, 0)
        self.ball = Ball(pygame.image.load(os.path.join('Sprites\\ball.png')).convert_alpha(), 400, 300, 16, 16, ponggame.vectorX, ponggame.vectorY, ponggame.ball_speed, self.plat_1, self.plat_2, False, False)
        self.ai = AI(self.ball)
        self.rebounds_median = 0
        self.counter = 0

    def norm(self, array):
        min_arr = np.array((0, 0, 0, -ponggame.ball_speed, -int(ponggame.ball_speed/2)), dtype=np.float32)
        max_arr = np.array((500, 800, 600, ponggame.ball_speed, int(ponggame.ball_speed/2)), dtype=np.float32)
    ...
    def reset(self):
        ...
        res = self.norm(np.array([self.plat_1.y, self.ball.x, self.ball.y,
                        self.ball.vectorX * ponggame.ball_speed,
                        self.ball.vectorY * ponggame.ball_speed], dtype=np.float32)) #Ракетка 1 Y Ракетка 2 Y
    ...
    def step(self, action):
        ...
        if action == 1 and (self.plat_1.y) >= 0:
            self.plat_1.move_up()
            reward = reward_move
        if action == 2 and (self.plat_1.y + self.plat_1.size_y) <= ponggame.disp_height:
        ...
        if self.ball.y + self.ball.size_y > ponggame.disp_height and self.ball.vectorY > 0:
        ...
        self.last_observation = self.norm(np.array(
            [self.plat_1.y, self.ball.x, self.ball.y, self.ball.vectorX * ponggame.ball_speed,
             self.ball.vectorY * ponggame.ball_speed],
            dtype=np.float32))
